...
import 'package:orbit_music/services/file_storage.dart';
...
List<SettingItem> audioandplaybackScreenData(BuildContext context) => [
  ...
  if (Platform.isAndroid)
    SettingItem(
      title: S.of(context).App_Folder,
      icon: CupertinoIcons.folder,
      subtitle: (context) {
        return ValueListenableBuilder(
          valueListenable: Hive.box(
            'SETTINGS',
          ).listenable(keys: ['APP_FOLDER']),
            ...
            Directory? newDirectory = await FolderPicker.pick(
              ...
              rootDirectory: Directory(
                _box.get(
                      'APP_FOLDER',
                      defaultValue:
                          GetIt.I<FileStorage>().storagePaths.basePath,
                    ) ??
  ...
  if (!Platform.isWindows)
    SettingItem(
      ...
      onTap: (context) async {
        await GetIt.I<MediaPlayer>().skipSilence(
          !(context.read<SettingsManager>().skipSilence),
        );
      },
      trailing: (context) {
  ...
  SettingItem(
    title: S.of(context).Crossfade,
    icon: CupertinoIcons.waveform,
    subtitle: (context) {
      int duration = context.watch<SettingsManager>().crossfadeDuration;
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
            duration == 0 ? "Desativado" : "${duration}s",
            style: TextStyle(
          ...
          Slider(
            ...
            onChanged: (value) {
              context.read<SettingsManager>().crossfadeDuration = value.toInt();
            },
          ),
        ],
      );
  ...
  SettingItem(
    ...
    trailing: (context) {
      return ValueListenableBuilder(
        ...
        builder: (context, value, child) {
          bool isEnabled = value.get('PLAYBACK_HISTORY', defaultValue: true);
