...
import 'package:orbit_music/app_config.dart';
...
class _AppWrapperState extends State<AppWrapper> {
  ...
  Future<void> _initApp() async {
    ...
    if (Platform.isWindows || Platform.isLinux) {
      ...
      JustAudioMediaKit.title = 'Orbit Music';
  ...
  Widget build(BuildContext context) {
    if (!_isInitialized) {
      ...
      final int themeModeIndex = box.get('THEME_MODE', defaultValue: 0);
      final bool amoledBlack = box.get('AMOLED_BLACK', defaultValue: true);
      ...
      final Color bgColor = isDark 
          ? (amoledBlack ? Colors.black : const Color(0xFF121212)) 
          : Colors.white;
    ...
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(create: (_) => GetIt.I<SettingsManager>()),
        ChangeNotifierProvider(create: (_) => GetIt.I<MediaPlayer>()),
        ChangeNotifierProvider(create: (_) => GetIt.I<LibraryService>()),
      ],
      child: const OrbitMusic(),
    );
  }
}

class OrbitMusic extends StatelessWidget {
  const OrbitMusic({super.key});
  @override
  Widget build(BuildContext context) {
