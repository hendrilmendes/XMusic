...
mixin SearchMixin on YTMusicServices {
  Future<List<Map<String, dynamic>>> getSearchSuggestions(
    ...
    if (query == '') {
      return Hive.box('SEARCH_HISTORY').values
          .toList()
    ...
    List<Map<String, dynamic>> suggestions = Hive.box('SEARCH_HISTORY').values
  ...
  Map<String, dynamic> _handleMusicCardShelfRenderer(Map item) {
    ...
    section.addAll(
      _handleHeader(nav(item, ['header', 'musicCardShelfHeaderBasicRenderer'])),
    );
    section['contents'] = [];
